...
function deleteRow(button) {
    ...
    var row = button.parentNode.parentNode;
    var id = row.getAttribute("data-id");
    var platillo = row.getElementsByTagName("td")[1].innerText;
    
    Swal.fire({
        title: 'Seguro?',
        text: 'Seguro que quiere borrar: '+ platillo + '?',
        icon: 'warning',
        confirmButtonText: 'Aceptar',
    }).then((result) => {
        if (result.isConfirmed) {
          // Si el usuario confirma, se procederá para el eliminado del platillo
          fetch('borrarPlatillo.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            ...
            .then(data => {
                if (data.success) {
                    Swal.fire({
                        title: 'Actualizado!',
                        text: 'EL platillo fue eliminado',
                        icon: 'success',
                        confirmButtonText: 'Aceptar',
                    }).then((result) => {
                        if (result.isConfirmed) {
                    ...
                    Swal.fire({
                        title: '¡Error!',
                        text: 'Hubo un error al borrar el platillo',
                        icon: 'error',
                        confirmButtonText: 'Aceptar'
                    });
                }
